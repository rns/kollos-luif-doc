...
whose semantic action or event handler is being called.
...
the alternative, whose semantic action or event handler is being called, relative to other alternatives or nil if no precedence is defined for the alternative.

##### `pos, len = luif.context.span()`

returns position and length of the input section corresponding to
the BNF rule whose semantic action or event handler is being called.
